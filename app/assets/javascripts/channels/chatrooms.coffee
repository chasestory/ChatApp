App.chatrooms = App.cable.subscriptions.create "ChatroomsChannel",
  ...
  received: (data) ->
    ...
    if active_chatroom.length > 0
      ...
      active_chatroom.append("<div><i style=' margin-left:3px; margin-right:3px;'>#{today_date}</i><i style='margin-right:7px;'>#{today_time}</i><span style='border-right:1px solid black;'></span><strong style='margin-left:5px;'>#{data.username}:</strong> #{data.body}</div>")
